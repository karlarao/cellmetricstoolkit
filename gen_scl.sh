...
for i in ${!cell_*}; do
  ...
  echo "/usr/local/bin/dcli -l root -g /root/cell_group -f ~/$i.scl" >> distribute_scl.sh
  echo "/usr/local/bin/dcli --serial -l root -g /root/cell_group "cellcli -e start $i.scl" | bzip2 > $i.txt.bz2" >> run_scl.sh
done
